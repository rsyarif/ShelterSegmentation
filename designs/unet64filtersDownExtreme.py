...
def build():
    ...
    conv2 = Conv2D(32, (3, 3), activation='relu', padding='same')(pool1)  # -> convolution to  features: 64     window: 18
    conv2 = Conv2D(32, (3, 3), activation='relu', padding='same')(conv2)  # -> convolution to  features: 64     window: 18
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)                         # -> maxpool to      features: 64     image : x / (2^2)

    conv3 = Conv2D(16, (3, 3), activation='relu', padding='same')(pool2) # -> convolution to  features: 128    window: 54 
    conv3 = Conv2D(16, (3, 3), activation='relu', padding='same')(conv3) # -> convolution to  features: 128    window: 162
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)                         # -> maxpool to      features: 128    image : x / (2^3)

    conv4 = Conv2D(8, (3, 3), activation='relu', padding='same')(pool3) # -> convolution to  features: 256    window: 486
    conv4 = Conv2D(8, (3, 3), activation='relu', padding='same')(conv4) # -> convolution to  features: 256    window: 1458
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)                         # -> maxpool to      features: 256    window: ..

    conv5 = Conv2D(4, (3, 3), activation='relu', padding='same')(pool4) # -> convolution to  features: 512    window: ..
    conv5 = Conv2D(4, (3, 3), activation='relu', padding='same')(conv5) # -> convolution to  features: 512    image : x / (2^4)


    # deconvolution
    up6     = Conv2DTranspose(8, (2, 2), strides=(2, 2), padding='same')(conv5) # deconv to  features: 512    x / (2^4)
    concat6 = concatenate([up6, conv4], axis=3)                                   # add conv4
    conv6   = Conv2D(8, (3, 3), activation='relu', padding='same')(concat6)     # convolute
    conv6   = Conv2D(8, (3, 3), activation='relu', padding='same')(conv6)      # convolute

    up7     = Conv2DTranspose(16, (2, 2), strides=(2, 2), padding='same')(conv6)
    concat7 = concatenate([up7, conv3], axis=3)
    conv7   = Conv2D(16, (3, 3), activation='relu', padding='same')(concat7)
